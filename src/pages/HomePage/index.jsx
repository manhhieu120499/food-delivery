...
const HomePage = () => {
  const [filterFood, setFilterFood] = useState([...foodList])

  const handleFilterFood = (index) => {
    let categorySelected = foodList[index];
    setFilterFood([categorySelected])
  }
  return (
    <>
      <Navbar />
      <div className={cx("wrapper-home")}>
        ...
        <section className={cx("explore")}>
          ...
          <div className={cx("explore-submenu")}>
            {exploreMenu.map((menu, index) => (
              <FoodCategory index={index} key={index} imageSrc={menu.src} name={menu.name} onClick={handleFilterFood}/>
            ))}
          </div>
        </section>
        ...
        <section className={cx("food-top")}>
          ...
          <div className={cx("food-list")}>
            {filterFood.map(food => food.menu.map((menu, index) =><FoodCard key={index} name={menu.name} pic={menu.pic} price={menu.price}/>))}
          </div>
        </section>
